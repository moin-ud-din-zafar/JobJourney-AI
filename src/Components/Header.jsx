...
export default function Header({ onMenuClick }) {
  ...
  // Simulated user data (normally fetched from the backend)
  const [user, setUser] = useState(null)

  useEffect(() => {
    setTimeout(() => {
      setUser({ firstName: 'John', lastName: 'Peterson', email: 'john.peterson@email.com' }) // Example user
    }, 1000)
  }, [])

  // Check if the current path is part of the profile section
  ...
  const light = {
    ...
    searchInput:
      'w-full rounded-lg py-2 pl-10 pr-4 border border-gray-200 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-100',
    iconBtn: 'p-2 bg-gray-100 rounded-lg transition',
    icon: 'text-gray-700',
    notifWrap: 'relative p-2 bg-gray-100 rounded-lg',
    nameText: 'text-lg font-bold text-gray-900', // Text color for name in light mode
  }
  const dark = {
    ...
    searchInput:
      'w-full rounded-lg py-2 pl-10 pr-4 border border-slate-700 bg-slate-900 text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-teal-100',
    iconBtn: 'p-2 bg-slate-800 rounded-lg transition',
    icon: 'text-gray-200',
    notifWrap: 'relative p-2 bg-slate-800 rounded-lg',
    nameText: 'text-lg font-bold text-gray-100', // Text color for name in dark mode
  }
      ...
      <header className={s.header}>
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 bg-teal-600 rounded-full flex items-center justify-center text-white font-bold">JT</div>

            {/* Conditionally render the user's name or logo */}
            {isProfilePage && user ? (
        ...
        {/* Only show search bar if not on profile page */}
        {!isProfilePage && (
