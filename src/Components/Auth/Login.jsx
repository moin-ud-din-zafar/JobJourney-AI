...
export default function Login({ onLogin }) {
  ...
  useEffect(() => {
    // show verification or signup success messages from query params
  ...
  const handleSubmit = async e => {
    ...
    setSubmitting(true);
    try {
      // use AuthContext login so app updates user/token globally
      await auth.login({ email: form.email.trim(), password: form.password });
      // optional parent callback
      if (typeof onLogin === 'function') onLogin();
      // go to dashboard or home (replace history so back won't return to login)
      navigate('/', { replace: true });
    } catch (err) {
      // backend error format: err.response.data.error
      const apiErr = err?.response?.data?.error || err?.message || 'Sign in failed';
      setMessage(`❌ ${apiErr}`);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-md p-8 lg:p-6 lg:max-w-lg lg:scale-90 lg:space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <div className="relative">
            ...
            <Link to="#" className="absolute right-0 top-0 mt-1 text-sm text-indigo-600 hover:underline">
              Forgot password?
            </Link>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className={`w-full py-2 rounded text-white font-semibold ${submitting ? 'bg-indigo-300' : 'bg-teal-600 hover:bg-teal-700'}`}
          >
